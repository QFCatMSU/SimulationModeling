...
lw <- c(.01,3)       # checked
natmort <- c(0.05,0.1,0.15,0.2,0.25,0.3,0.35) #range of mortality values (not instantaneous rates)
...
#For assignment 2.5: Add a loop over mortality values as per assignment task
final_pop <- numeric(7)
for (m in 1:7) {
...
btrout[1,1] <- 4000
for (j in 2:15) {
  btrout[j,1] <- btrout[j-1,1]*(1-natmort[m])
}
...
for (i in 1:99)  {
  ...
  #Step 8: Caclulate total deaths, use non-instantaneous rates, as per VisBasic model
  deaths <- natmort[m] * btrout[,i] + catch
  
  #Step 9: Apply tautology
  for (j in 15:2) {
    btrout[j,i+1] <- btrout[j-1,i] - deaths[j-1] 
    btrout[1,i+1] <- age0 * (1 - natmort[m])  #first year losses, no fishing
  } 
...
final_pop
plot(final_pop~natmort, log='y', type='l',)
